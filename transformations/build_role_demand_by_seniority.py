...
def main():
    ...
    analytics_key = (
        f"analytics/role_demand_by_seniority/v1/dt={dt}/"
        f"role_demand_by_seniority_{dt}.parquet"
    )
    local_output = f"/tmp/role_demand_by_seniority_{dt}.parquet"
    ...
    df = df[df["seniority_level"].notna()]
    df = df[df["seniority_level"] != "unknown"]

    print("Rows after filters:", len(df))

    result = (
        df.groupby(["role_family", "seniority_level"])
        .size()
        .reset_index(name="demand")
        .sort_values(["role_family", "demand"], ascending=[True, False])
    )

    print("\n[ROLE DEMAND BY SENIORITY SUMMARY]")
